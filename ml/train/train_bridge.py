"""
Training pipeline for BRIDGE asset type.
Uses real bridge_dataset.csv from Dev A.

Trains:
  - XGBoost risk scorer     → ml/models/risk_model_BRIDGE.pkl
  - SHAP explainer          → ml/models/shap_explainer_BRIDGE.pkl
  - StandardScaler          → ml/models/scaler_BRIDGE.pkl
  - LSTM Autoencoder (GPU)  → ml/models/lstm_autoencoder_BRIDGE.pt

Run: python -m ml.train.train_bridge
"""
...
CSV_PATH   = "data/raw/bridge_sensor_data.csv"
MODEL_DIR  = "ml/models"
ASSET_TYPE = "BRIDGE"
...
def load_data():
    ...
    # Synthetic data already has binary label column — no mapping needed
    ...
    print(f"   Normal     : {total - failures} ({(total-failures)/total*100:.1f}%)")
...
def preprocess(df):
    ...
    # Clip outliers at 3 std per feature
    for col in FEATURES:
...
def train_xgboost(X_train, X_test, y_train, y_test):
    ...
    model = XGBClassifier(
        ...
        min_child_weight=3,
        gamma=0.1,
        scale_pos_weight=ratio,     # handles Severe class imbalance
        eval_metric="auc",
...
# ── 5. Train LSTM Autoencoder on GPU ──────────────────────────────────────────
...
def train_lstm(X_train_scaled, X_test_scaled, y_test):
    ...
    try:
        ...
        # Train LSTM on normal (non-failure) samples only
...
def main():
    print("=" * 55)
    print(f"  InfraWatch — BRIDGE Asset Type Training Pipeline")
    print("=" * 55)
